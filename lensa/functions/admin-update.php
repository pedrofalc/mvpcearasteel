...
/* CoLabsFramework Version Getter */
/*-----------------------------------------------------------------------------------*/
function colabs_get_fw_version($url = '', $type = 'theme', $check_if_critical = false){
  ...
  if(!is_wp_error($temp_file_addr) && $file_contents = file($temp_file_addr)) {
    ...
    if($type == 'framework'){
      ...
      $output['version'] = COLABS_THEME_VER;
    }
  }
...
function colabs_framework_version_checker( $local_version, $check_if_critical = false ) {
	...
	$version_data = colabs_get_fw_version( '', 'framework', $check_if_critical );
