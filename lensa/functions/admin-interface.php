...
function colabsthemes_add_admin() {
  ...
  if(get_option( 'framework_colabs_backend_icon')) { 
    ...
    if(!file_exists($icon)) $icon = get_template_directory_uri() . '/functions/images/shortcode-icon.png'; 
  }
...
function colabsthemes_machine($options) {
  ...
  foreach ($options as $value) {
    ...
    case "heading":
      ...
      $jquery_click_hook = sanitize_title($value['name']);
      $jquery_click_hook = "colabs-option-" . $counter;
...
function colabs_theme_panel_css(){
  ...
  if('light'==$color_scheme){
    ...
    $panel_style .= "#header-nav li.current-page i.icon, #header-nav li a:hover i.icon {color: #FFF;}";
  }else{
